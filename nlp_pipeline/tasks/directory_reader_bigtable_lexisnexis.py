...
from nlp_pipeline.settings import EXCLUDE_LEXISNEXIS_TOPICS, LSH_SEED, MIN_JACCARD_DR, environment
...
class DirectoryReaderBigtableLexisnexis(DirectoryReaderLexisNexis):
    name = "Directory Reader Bigtable Lexisnexis"
    source_type = "lexisnexis"
    extension = ".jsonl"
    pipeline_name = "bigtable_lexisnexis"
    compression_type = "gz"
    ...
    def extract_metadata(self, article: Dict) -> Dict:
        ...
        return {"metadata": {**metajson_obj, "extra_lexis_nexis_info": {**article}}}
                ...
                with open(f"{input_dir_path}/{file_name}", encoding="utf-8", errors="ignore") as file:
                    ...
                    for line in file:
                        ...
                        if self.has_excluded_topic(topics, EXCLUDE_LEXISNEXIS_TOPICS):
                            logger.debug("skipping article due to topic")
                            skipped += 1
                            continue
                        elif not text:
                            logger.debug("skipping article due to text being empty")
                            skipped += 1
                            continue
...
app.register_task(DirectoryReaderBigtableLexisnexis())
