...
from nlp_pipeline.tasks.publish_product_to_gcp import GcpTablePublisher, PublishProductToGcp
...
class TestGcpTablePublisher:
    ...
    def test_format_data(self, article_fixture, expected_source, expected_source_type, request):
        ...
        sb_pub = GcpTablePublisher(session_scope, NodeProduct)
    ...
    def test_get_primary_keys(self):
        publisher = GcpTablePublisher(session_scope, NodeProduct)
        primary_keys = publisher._get_primary_keys()

        assert primary_keys == ["vid", "product", "product_source"]
...
def test_run(article_batch_with_products_1, test_db):
    result = PublishProductToGcp().run(article_batch_with_products_1)
